//Server code for FRA application
...
//Trasnfer data from server to client
...
int main(int argc, char **argv)
{
	...
	gpio_3 = mmap(NULL, sysconf(_SC_PAGESIZE),PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0x41210000);	
	...
	while(1)
	{
		...
		else if (strncmp(buffer_word, "SADC", 4) == 0)
		{								
			//Reset FIFOs
			rst_FIFOs(gpio_2);
		...
		//Transfer acquired data from server to client
...
{	
	char ack[4];	
	char data_ch1_str[12];
	char data_ch2_str[12];		
	
	for (int i = 0; i < 64; i++)
	{		
		//Channel 1 transference		
		bzero(ack,4);					
		recv(newsockfd,ack,4,0);		
		if(strncmp(ack, "RDY_", 4) == 0)
		{					
			bzero(ack,4);			
			send(newsockfd,&data_ch1[i],sizeof(int),0);										
		}				
				
		//Channel 2 transference				
		bzero(ack,4);			
		recv(newsockfd,ack,4,0);					
		if(strncmp(ack, "RDY_", 4) == 0)
		{		
			bzero(ack,4);									
			send(newsockfd,&data_ch2[i],sizeof(int),0);				
		}		
	}	
}
